...
class Room {
  ...
  async broadcast(data) {
    for (let member of this.members) {
      if (data.text == "/joke" && data.name == member.name) {
        const response = await member.getJoke();
        data.text = response;
        member.send(JSON.stringify(data));
        break;
      } else if (data.text == "/members" && data.name == member.name) {
        const response = this.getMembers();
        member.send(JSON.stringify(response));
        break;
      } else if (data.text != "/joke" && data.text != "/members") {
        member.send(JSON.stringify(data));
      }
    }
  }
